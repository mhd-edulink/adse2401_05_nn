...
def load_and_preprocess_image(image_path: Path, image_size: int) -> Tensor:
    ...
    if not image_path.exists():
        raise FileNotFoundError(
            f"Could not find input image at '{image_path}'"
...
class Encoder(nn.Module):
    ...
    def __init__(
        self, image_size: int, number_of_channels: int, latent_dimension: int
    ) -> None:
...
class Decoder(nn.Module):
    ...
    def forward(self, latent_vector_batch: Tensor) -> Tensor:
        ...
        reconstructed_images = flat_reconstruction.view(
            batch_size, self.number_of_channels, self.image_size, self.image_size
        )
...
def compute_vae_loss(
    ...
    reconstruction_loss = functional.mse_loss(
        reconstructed_images, original_images, reduction="sum"
    )
...
def display_and_save_comparison(
    original_image_tensor: Tensor, reconstructed_image_tensor: Tensor, output_path: Path
) -> None:
...
def main() -> None:
    ...
    try:
        original_image_tensor = load_and_preprocess_image(
            ORIGINAL_IMAGE_PATH, IMAGE_SIZE
        )
    ...
    print(
        f"Image loaded and resized to {IMAGE_SIZE} x {IMAGE_SIZE}."
    ...
    print(
        f"By Training with only one training image, the network is intentionally "
    ...
    final_reconstruction_loss = train_vae_on_single_image(
        vae_model,
    ...
    print(
        f"Original image dimensions: {IMAGE_SIZE} x {IMAGE_SIZE}"
